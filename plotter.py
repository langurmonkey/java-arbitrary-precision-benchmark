...
def plot_jmh_results(data: List[Dict], name: str):
    ...
    for i, (benchmark_name, param_data) in enumerate(benchmark_groups.items()):
        ...
        for j, (precision, score, error) in enumerate(param_data):
            ...
            ax.errorbar(score, group_offset + j, xerr=error * 0.1, capsize=2, fmt='none', ecolor='#999999', elinewidth=0.5)
            ax.text(score + 0.01, group_offset + j, f'{precision}', va='center', fontsize=7)
    ...
    ax.set_yticklabels(ytick_labels, fontsize=7, rotation=45, ha='right')
    ax.set_xlabel('Score [ns/op] (lower is better)', fontsize=9)
    ax.set_title(f'Apfloat v BigDecimal benchmark: {name}', fontsize=10)
    ...
    ax.legend(handles=legend_patches, fontsize=7, title='Precisions')
